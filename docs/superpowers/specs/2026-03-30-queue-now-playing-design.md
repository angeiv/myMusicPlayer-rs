...
- 移除语义约束（MVP）：
  - 移除非当前项 **不应改变当前正在播放的 track**（不 stop、不自动跳转）。
  - next/previous/auto-next 的行为应等价于该项从未存在。
  - 为避免索引漂移导致 previous 异常，MVP 允许在移除后清空 `history`（不要求保留精确的播放历史）。
...
- MVP 实现允许的等价效果：
  ...
  - 队列 UI **以空列表为准**：显示空态文案（例如“队列已清空”）；但必须满足“不打断当前播放”。
