...
impl PlaylistService {
    ...
    pub fn create_playlist(&mut self, name: &str) -> Result<Uuid, String> {
        ...
        if self.playlist_names.contains_key(name) {
        ...
        self.playlist_names.insert(name.to_string(), id);
    ...
    pub fn add_to_playlist(&mut self, playlist_id: &Uuid, track: Track) -> Result<(), String> {
    ...
    pub fn remove_from_playlist(&mut self, playlist_id: &Uuid, track_index: usize) -> Result<Option<Track>, String> {
        if let Some(playlist) = self.playlists.get_mut(playlist_id) {
        ...
        if let Some(playlist) = self.playlists.get_mut(id) {
            if let Some(new_name) = name {
                if !new_name.trim().is_empty() && new_name != playlist.name {
                    ...
                    if self.playlist_names.contains_key(new_name) {
                    ...
                    self.playlist_names.insert(new_name.to_string(), *id);
                }
            }
...
mod tests {
    ...
    fn create_test_track() -> Track {
        Track {
            ...
            path: PathBuf::from("/path/to/track.mp3"),
            duration: 180,
            ..Default::default()
        }
    }
    ...
    fn test_add_and_remove_tracks() {
        ...
        // Add a track
        assert!(service.add_to_playlist(&playlist_id, track).is_ok());
    ...
    fn test_update_playlist_metadata() {
        ...
        // Update name
        assert!(service.update_playlist_metadata(
            &playlist_id, 
            Some("New Name"), 
        ...
        let result = service.update_playlist_metadata(
            &another_id,
            Some("New Name"),
            None
        );
